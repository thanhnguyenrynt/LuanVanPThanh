...
\begin{figure}[H]
	\centering
	\includegraphics[width=9cm,height=9cm,keepaspectratio]{mosiac.png}
...
SSD sử dụng mô mạng CNN duy nhất, kiến trúc gồm hai phần với một mạng cơ sở có thể là VGG16, ResNet hay Mobinet được loại bỏ tầng kết nối đầy đủ có nhiệm vụ trích xuất ra các bản đồ đặc trưng, một tập hợp các tầng tích chập phụ trợ để thay thế cho tầng kết nối bị loại bỏ. 
...
\begin{algorithm}
	...
	\KwData{
	$B = {b_1, ..., b_n}, P = {p_1, ..., p_n}, \lambda_p, \lambda_{IoU}$ \\
	$B$ là danh sách các hộp giới hạn \\
	$P$ chứa điểm đối tượng tương ứng \\
	$\lambda_p$ xác định ngưỡng đối tượng \\
	$\lambda_{IoU}$ là ngưỡng IoU\\
	}
	\KwResult{
		$B_r = \{\}, P_r = \{\}$ \\
